...
def f(s):
    ...
    if all(f(m) == 'p1' for m in moves(s)):   # №19 меняем здесь all на any, ищем b1 по условию
        return 'b1'
    if any(f(m) == 'b1' for m in moves(s)):  # №20 в (10 строка) меняем обратно на all, ищем p2 по условию
        return 'p2'
    if all(f(m) == 'p1' or f(m) == 'p2' for m in moves(s)):  # №21 в (10 строка) оставляем all, ищем b1/2 по условию
        return 'b1/2'
